...
<body <?php body_class(); ?>>
	...
	<a class="skip-link screen-reader-text" href="#primary"><?php esc_html_e('Skip to content', 'magic-elementor-lite'); ?></a>
	<?php
	...
	if ($magic_elementor_use_htemplate && did_action('elementor/loaded')) {
		...
		if ('select' == $magic_elementor_template_list) {
			printf('<div class="me-htemplate-not">%s</div>', __('Please select a template for display Header!! If the template list is empty then you need to add a header template first!!!', 'magic-elementor-lite'));
		} else {
	...
	?>
		<header class="header <?php if (!has_nav_menu('main-menu')) : ?>mg-nomenu<?php endif; ?>" id="header">
			...
			<?php do_action('magic_elementor_lite_header'); ?>
		</header>
	<?php
	}
	?>